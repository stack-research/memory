...
.PHONY: synth deploy exp-e1 exp-e2 exp-e3 exp-e4 exp-e5 exp-e6 exp-e7 exp-e8 exp-e9 exp-e10 exp-e11 exp-e12 ingest ingest-preflight show-tables e2-analysis e5-analysis e9-analysis e10-analysis e11-analysis e12-analysis phase4-audit phase5-audit phase6-audit phase6-regression lab-regression e9-e12-regression implicit-regression implicit-regression-aws im-trigger-analysis im-contamination-analysis im-replay-diff im-l im-m im-n im-o im-p im-q im-r im-s im-t im-u im-v im-w im-w-loop-probe axis-dominance-audit
...
im-r:
	cd $(STACKS_DIR) && AWS_PROFILE=$(AWS_PROFILE) AWS_REGION=$(AWS_REGION) PYTHONPATH=$(PYTHONPATH_ROOT) uv run python -m src.experiments.implicit.im_r_provenance_signal_writer

im-s:
	cd $(STACKS_DIR) && AWS_PROFILE=$(AWS_PROFILE) AWS_REGION=$(AWS_REGION) PYTHONPATH=$(PYTHONPATH_ROOT) uv run python -m src.experiments.implicit.im_s_provenance_writer_closeout

im-t:
	cd $(STACKS_DIR) && AWS_PROFILE=$(AWS_PROFILE) AWS_REGION=$(AWS_REGION) PYTHONPATH=$(PYTHONPATH_ROOT) uv run python -m src.experiments.implicit.im_t_tai_timekeeping

im-u:
	cd $(STACKS_DIR) && AWS_PROFILE=$(AWS_PROFILE) AWS_REGION=$(AWS_REGION) PYTHONPATH=$(PYTHONPATH_ROOT) uv run python -m src.experiments.implicit.im_u_epistemic_triangle

im-v:
	cd $(STACKS_DIR) && AWS_PROFILE=$(AWS_PROFILE) AWS_REGION=$(AWS_REGION) PYTHONPATH=$(PYTHONPATH_ROOT) uv run python -m src.experiments.implicit.im_v_control_plane_ingest

im-w:
	cd $(STACKS_DIR) && AWS_PROFILE=$(AWS_PROFILE) AWS_REGION=$(AWS_REGION) PYTHONPATH=$(PYTHONPATH_ROOT) uv run python -m src.experiments.implicit.im_w_runtime_calibration

im-w-loop-probe:
	cd $(STACKS_DIR) && AWS_PROFILE=$(AWS_PROFILE) AWS_REGION=$(AWS_REGION) IMPLICIT_CALIBRATION_PROFILE=loop_probe PYTHONPATH=$(PYTHONPATH_ROOT) uv run python -m src.experiments.implicit.im_w_runtime_calibration

axis-dominance-audit:
	cd $(STACKS_DIR) && AWS_PROFILE=$(AWS_PROFILE) AWS_REGION=$(AWS_REGION) PYTHONPATH=$(PYTHONPATH_ROOT) uv run python -m src.experiments.run_sql_file ../src/experiments/sql/axis_dominance_audit.sql --database memory_lab --workgroup memory-lab
