...
_EXPERIMENT_TIME_CONTEXT_ID = "exp-in-memory-bootstrap-v6"


@dataclass
class InMemoryLineageEngine:
    """In-memory v6 lineage emitter for the implicit experiment suites.

    Mirrors `LineageEngine` semantics: per-stream sequence + HLC, full
    `physical_moment` block on every event, no wall-clock reads.
    """
    ...
    def emit(
        ...
        *,
        ...
        self._seq_by_stream[stream_id] = self._seq_by_stream.get(stream_id, 0) + 1
